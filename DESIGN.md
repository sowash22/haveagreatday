...
The whole planner fits in one normal viewport. The card contains, in order: the brand with one quiet purpose line, place search with an adjacent temperature-unit toggle, a compact seven-day ribbon with day names only, one selected day, one personal note that flows through up to three schedule-friendly windows, and a collapsed method disclosure. The ranking remains strongest to third strongest, but reads as natural prose rather than segmented results. Times are emphasized inline while weather, light, temperature, AQI, and UV follow as quiet evidence in the same sentence. The app evaluates Morning, Noon, Evening, and Night, then keeps three; Morning, Evening, and Night win ties so Noon appears only when its forecast fit is better. The inference service may vary the opening and connective phrases, while the application always injects the ranked times and evidence. There is no app bar, page footer, activity control, alternatives section, preferences form, AI badge, or chat-interface chrome on the primary surface. One compact hourly-fit chart belongs inside the disclosure, never on the main decision surface.
