...
The whole planner fits in one normal viewport. The card contains, in order: the brand with one quiet purpose line, place search with an adjacent temperature-unit toggle, a compact seven-day ribbon with day names only, one selected day, one adaptive personal note, and two adjacent disclosures for the evidence and founder story. The note has three possible shapes: an all-day message with whole-day evidence plus two or three practical daylight windows, two or three ranked daylight windows with inline evidence, or an honest recommendation to choose another day. A brief closing note explains why other dayparts ranked lower or were omitted. Window ranking remains strongest first and reads as natural prose rather than segmented results. Deterministic guardrails decide which shapes and windows the forecast can support; the inference service chooses the most practical allowed shape and connective language. The application always injects the measurements. There is no app bar, page footer, activity control, alternatives section, preferences form, AI badge, or chat-interface chrome on the primary surface. One compact hourly-fit chart belongs inside the evidence disclosure, never on the main decision surface. The founder disclosure uses the same glass sheet and keeps the personal story optional.
